...
from app.repositories import user_repository, session_repository, audit_repository
...
async def get_auth_service(db: AsyncSession = Depends(get_db)) -> AuthService:
    ...
    audit_repo = audit_repository.AuditRepository(db)
...
def ensure_is_admin_or_facility_manager(
    ...
    if current_user.role not in [UserRoles.ADMIN, UserRoles.FACILITY_MANAGER]:
        logger.warning(
            "admin_or_facility_manager_access_denied", user_id=current_user.id
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions"
        )
